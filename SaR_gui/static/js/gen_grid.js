...
function draw(state, world_settings, new_messages, accessible_chatrooms, new_tick) {
    ...
    // Update agent thinking status indicator in toolbar
    var agent_status_el = document.getElementById("agent_status");
    if (agent_status_el) {
        var thinking_found = false;
        var status_text_map = {
            'thinking_planning': 'RescueBot: Planning...',
            'thinking_reasoning': 'RescueBot: Deciding...'
        };
        Object.keys(state).forEach(function(objID) {
            if (objID === "World") return;
            var obj_data = state[objID];
            if (obj_data.hasOwnProperty('thinking_state') && obj_data.hasOwnProperty('isAgent')) {
                var ts = obj_data['thinking_state'];
                if (ts && ts !== 'idle' && status_text_map[ts]) {
                    agent_status_el.style.display = '';
                    agent_status_el.textContent = status_text_map[ts];
                    thinking_found = true;
                }
            }
        });
        if (!thinking_found) {
            agent_status_el.style.display = 'none';
            agent_status_el.textContent = '';
        }
    }
